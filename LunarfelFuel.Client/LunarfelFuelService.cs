...
using Lunarfel.LunarfelFuel.Client.Helpers;
using System.Linq;
using Lunarfel.LunarfelFuel.Client.Deps;

namespace Lunarfel.LunarfelFuel.Client
{
	[PublicAPI]
	public class LunarfelFuelService : Service
	{
		...
		public LunarfelFuelService(ILogger logger, ITickManager ticks, ICommunicationManager comms, ICommandManager commands, IOverlayManager overlay, User user) : base(logger, ticks, comms, commands, overlay, user) { }

		public override async Task Started()
		{
			// Request server configuration
			this.config = await this.Comms.Event(LunarfelFuelEvents.Configuration).ToServer().Request<Configuration>();
            ...
            else
            {
                positions.ToList().ForEach(position =>
                {
                    if (!pickups.Any(pickup => position.DistanceToSquared(pickup.Position) < 5f))
                    {
                        ...
                        int pickupHandle = API.CreatePickup(
        ...
        public Vector3 GetVehicleTankPos(Vehicle vehicle)
        {
            ...
            {
                var boneIndex = API.GetEntityBoneIndexByName(vehicle.Handle, boneName);
        ...
        {
            return API.GetVehicleHandlingFloat(vehicle.Handle, "CHandlingData", "fPetrolTankVolume");
        }
